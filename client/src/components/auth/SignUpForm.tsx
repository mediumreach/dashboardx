  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5">
      ...
      <div className="space-y-2">
        ...
        <div className="relative group">
          <div className={`absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none transition-colors duration-300 ${
            focusedField === 'fullName' ? 'text-cyan-600' : 'text-gray-400'
          }`}>
            <User size={20} />
          </div>
          <input
            ...
            required
            className="w-full pl-12 pr-4 py-3 border-2 border-gray-200 rounded-xl focus:ring-2 focus:ring-cyan-500 focus:border-transparent transition-all duration-300 bg-white hover:border-gray-300 hover:shadow-md focus:shadow-lg placeholder-gray-400"
            placeholder="John Doe"
          />
          {focusedField === 'fullName' && (
            <div className="absolute inset-0 rounded-xl bg-gradient-to-r from-blue-500/10 via-blue-500/10 to-blue-600/10 pointer-events-none animate-pulse-slow"></div>
          )}
        </div>
      </div>
      ...
      <div className="space-y-2">
        ...
        <div className="relative group">
          <div className={`absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none transition-colors duration-300 ${
            focusedField === 'tenantName' ? 'text-cyan-600' : 'text-gray-400'
          }`}>
            <Building2 size={20} />
          </div>
          <input
      ...
      <div className="space-y-2">
        ...
        <div className="relative group">
          <div className={`absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none transition-colors duration-300 ${
            focusedField === 'email' ? 'text-cyan-600' : 'text-gray-400'
          }`}>
            <Mail size={20} />
          </div>
          <input
      ...
      <div className="space-y-2">
        ...
        <div className="relative group">
          <div className={`absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none transition-colors duration-300 ${
            focusedField === 'password' ? 'text-cyan-600' : 'text-gray-400'
          }`}>
            <Lock size={20} />
          </div>
          <input
      ...
      <button
        type="submit"
        disabled={loading}
        className="w-full relative group overflow-hidden px-6 py-4 bg-gradient-to-r from-cyan-600 via-blue-600 to-blue-700 text-white rounded-xl font-semibold shadow-lg shadow-cyan-500/30 hover:shadow-xl hover:shadow-cyan-500/40 disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-300 hover:scale-[1.02] active:scale-[0.98]"
      >
        {/* Shimmer Effect */}
